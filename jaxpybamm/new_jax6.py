...
def f_jvp_batch(args, batch_axes):
    ...
    if (
        ...
        out = []
        batch_indices = [ix for ix, b in enumerate(batch_axes[len(args) // 2:]) if b is not None]
        ...
        tangent_batch_axes[0] is not None
    ):
        ...
        raise Exception(f"Batching can occur over time, or differentiation over "
