...
def jaxify_solve(t, inputs, t_eval):
    logger.info("jaxify_solve: ", type(t), type(inputs), type(t_eval))
    ...
    if len(t) == 1:
        ...
        return (
            ...
            np.array(term_v_sens).squeeze())  # casadi.DM -> 2d -> 1d
...
def f(t, inputs, t_eval):
    logger.info("f: ", type(t), type(inputs), type(t_eval))
    bind_point = f_p.bind(t, inputs, t_eval)
    logger.debug("f [exit]: ", (type(bind_point), bind_point))
    return bind_point


@f_p.def_impl
def f_impl(t, inputs, t_eval):
    """Concrete implementation of Primitive"""
    logger.info("f_impl: ", type(t), type(inputs), type(t_eval))
    term_v, term_v_sens = jaxify_solve(t, inputs, t_eval)
...
def f_batch(args, batch_axes):
    ...
    logger.debug("f_batch [exit]: ", (type(t), t))
    # return f(t, np.array([params[0]]), t_eval), batch_axes[0]
    return f(t, params, t_eval), batch_axes[0]
...
def f_jvp(primals, tangents):
    ...
    x_dot_t, x_dot_params, x_dot_t_eval = tangents

    # Deal with Zero tangents
    def make_zero(prim, tan):
        return lax.zeros_like_array(prim) if type(tan) is ad.Zero else tan

    y = f(x_t, x_params, x_t_eval)  # primals_out = fun(*primals); (unnecessary call?)
    y_dot = f_jvp_p.bind(  # tangents_out = J.v
        x_t,
        x_params,
        x_t_eval,
        make_zero(x_t, x_dot_t),
        make_zero(x_params, x_dot_params),
        make_zero(x_t_eval, x_dot_t_eval)
    )
...
def f_jvp_abstract_eval(t, inputs, t_eval, t_dot, inputs_dot, t_eval_dot):
    logger.info(
        "f_jvp_abstract_eval: ",
        type(t), type(inputs), type(t_eval),
        type(t_dot), type(inputs_dot), type(t_eval_dot))
    y_dot_aval = jax.core.ShapedArray(t_dot.shape, t_dot.dtype)
    logger.debug("f_jvp_abstract_eval [exit]: ", (type(y_dot_aval), y_dot_aval))
    return y_dot_aval


def f_jvp_transpose(y_bar, t, inputs, t_eval, t_dot, inputs_dot, t_eval_dot):
    """JVP transpose rule
    ...
    """
    logger.info("f_jvp_transpose: ", type(y_bar), type(t), type(inputs), type(t_eval))
    # assert ad.is_undefined_primal(x_dot_dummy)
    x_bar = f_vjp_p.bind(t, inputs, t_eval, y_bar)
    ...
    return None, None, None, jnp.dot(y_bar, x_bar), None, None
...
@f_vjp_p.def_impl
def f_vjp_impl(t, inputs, t_eval, y_bar):
...
def f_vjp_batch(args, batch_axes):
    ...
    t, params, t_eval, unknown_eq_1 = args
    # concrete implemenatation provides native batching
    term_v, term_v_sens = jaxify_solve(t, inputs, t_eval)
...
k = 5
print("\neval with scalar t:")
print(f(t_eval[k], inputs=inputs, t_eval=t_eval))

print("\ngrad with scalar t:")
print(jax.grad(f)(t_eval[k], x, t_eval))

print("\nvalue_and_grad with scalar t:")
value_and_grad_f = jax.value_and_grad(f)
print(value_and_grad_f(t_eval[k], x, t_eval))

# Vector evaluation

print("\neval with vmap over t:")
vmap_f = jax.vmap(f, in_axes=(0, None, None))
print(vmap_f(t_eval, x, t_eval))

print("\ngrad with vmap over t:")
vmap_grad = jax.vmap(jax.grad(f), in_axes=(0, None, None))
print(vmap_grad(t_eval, x, t_eval))

print("\nvalue_and_grad with scalar t:")
vmap_vg = jax.vmap(jax.value_and_grad(f), in_axes=(0, None, None))
print(vmap_vg(t_eval, x, t_eval))

# Differentiate downstream expressions

# Original:
t = t_eval
term_v, term_v_sens = vmap_vg(t, x, t_eval)
...
def rms(t):
    return jnp.sum((vf(t, x, t_eval) - data) ** 2)


# If the JVP / transpose rules are wrong, then (x*x) and (x**2) will not match
def rms2(t):
    return f(t, x, t_eval) * f(t, x, t_eval)


def rms_squared(t):
    return f(t, x, t_eval) ** 2


def rms_sq_data(t):
    return jnp.sum((f(t, x, t_eval) - data[k]) ** 2)
...
if True:
    ...
    def sse_jax_jac(params, t_eval):
        def sse(t):
            vf = jax.vmap(f, in_axes=(0, None, None))
            return jnp.sum((vf(t, np.append(params, 3.), t_eval) - data) ** 2)
